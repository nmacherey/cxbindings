...
bool CXBindingsCppHandlersGenerator::DoGenerateCode( CXBindingsGeneratorOptions& options )
{
	...
	for( ; it != m_objectInfos.end() ; ++it )
	{
		...
		{
			std::string srcFile = srcDir + "/") + m_objectFiles[it->first] + "XmlHandler"  + wxT(".cpp" ;
			std::string hdrFile = hdrDir + "/") + m_objectFiles[it->first] + "XmlHandler"  + wxT(".h" ;
...
void CXBindingsCppHandlersGenerator::DoGenerateCodeFor( CXBindingsObjectInfo& objectInfo , CXBindings& grammar , CXBindingsGeneratorOptions& options )
{
	...
	objectNodeName.Replace("Type"),"" ,true ;
...
CXBindingsHandlerFileInfo CXBindingsCppHandlersGenerator::DoGenerateRuleCodeFor( CXBindingsRuleInfo& ruleInfo , CXBindings& WXUNUSED(grammar) , CXBindingsGeneratorOptions& options )
{
	CXBindingsHandlerFileInfo res;
...
CXBindingsHandlerFileInfo CXBindingsCppHandlersGenerator::DoGenerateForEachChildRuleCodeFor( CXBindingsChildContainerInfo& WXUNUSED(childContainer) , CXBindingsForEachChildInfo& ruleInfo , CXBindings& grammar , CXBindingsGeneratorOptions& options )
{
	CXBindingsHandlerFileInfo ret;
...
void CXBindingsCppHandlersGenerator::DoGetIncludesListFor( CXBindingsArrayString& local_includes , CXBindingsArrayString currentObjects , CXBindingsHandlerFileInfo& file  , CXBindingsGeneratorOptions& options)
{
	...
	{
		if( !m_objectFiles[file.dependencies[i].second] ==options.ns + "_globals")) && !m_objectFiles[file.dependencies[i].second].IsEmpty(   {
...
std::string CXBindingsCppHandlersGenerator::FindRealTypeFor( const std::string& type )
{
	...
	if( newType =="std::string")) || newType =="std::string") || newType.IsSameAs(wxT("string"   
		newType = "string" ;
		...
		newType = "any" ;
	else if( newType =="unsigned int")) || newType.IsSameAs("uint"   
		newType = "uint" ;
	else if( newType =="unsigned long")) || newType.IsSameAs("ulong"   
		newType = "ulong" ;
